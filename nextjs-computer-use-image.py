#!/usr/bin/env python3
"""
Build a Daytona sandbox Docker image for the *new-project-deepagent* flow that
ALSO supports Computer Use / visual GUI automation.

This is the fix for the "Computer-use functionality is not available" 503
seen from `screenshot-capture.ts` / `visual-critique-tool.ts`: the sandbox
those tools run in (`open-swe-vcpu1-mem2-disk7`, image
`fouadmahmoud28/syntera-nextjs-sandbox:0.1.0`) never had the desktop stack
that Daytona's toolbox needs to start a computer-use session. Meanwhile
`syntera-computer-use-v6` (built by `computer-use-image.py`) *does* have that
stack, but it is built from the generic `daytonaio/langchain-open-swe:0.1.0`
base — it does not carry the pre-baked Next.js template
(`syntera-nextjs-sandbox` bakes shadcn components, npm install, prisma
generate, etc. — see `automation/build-sandbox-image/build_sandbox_image.py`)
that `new-project-deepagent` relies on.

Both `syntera-nextjs-sandbox:0.1.0` and the computer-use base image are built
`FROM daytonaio/langchain-open-swe:0.1.0`, so this script just layers the
same Xvfb/xfce4/x11vnc/novnc/Firefox desktop stack from
`computer-use-image.py` on top of `syntera-nextjs-sandbox:0.1.0` instead of
the bare langchain-open-swe base. Result: one image with both the Next.js
template AND a working computer-use desktop.

This image extends fouadmahmoud28/syntera-nextjs-sandbox:0.1.0 and pre-installs
the full desktop environment stack required by Daytona Computer Use:
...
Usage (on GitHub Codespaces or any Linux machine with Docker):
    ...
    python3 nextjs-computer-use-image.py
...
# Configuration
# ─────────────────────────────────────────────────────────
# Base is the EXISTING Next.js sandbox image (already has the template,
# shadcn components, npm deps, prisma client baked in) — not the bare
# langchain-open-swe image computer-use-image.py starts from.
BASE_IMAGE = "fouadmahmoud28/syntera-nextjs-sandbox:0.1.0"
...
def generate_dockerfile():
    dockerfile = textwrap.dedent(f"""\
        ...
        # ── Sanity check: the Next.js template must still be here ──
        #    Confirms the base image's baked-in template survived the layer
        #    additions above (it should — none of this touches {SANDBOX_HOME}).
        RUN echo "=== Verifying Next.js template is intact ===" && \\
            find / -maxdepth 4 -iname "_template" -type d 2>/dev/null || true
...
def create_daytona_snapshot(api_key: str, snapshot_name: str, full_image: str):
    ...
    print(f"   Resources: 2 vCPU, 4 GiB memory, 10 GiB disk (matches the existing")
    print(f"              '{OLD_SNAPSHOT_NAME}' snapshot's actual resources)\n")
    ...
    try:
        ...
        print(f"\n  NOTE: the old '{OLD_SNAPSHOT_NAME}' snapshot was left untouched.")
        print(f"  Test sandboxes built from '{snapshot_name}' before deleting it.")
        return True
...
def main():
    print("=" * 60)
    print("  🖥️  Syntera Next.js + Computer Use Sandbox Image Builder")
    print("=" * 60)
    print(f"\n  Base:   {BASE_IMAGE}")
    print("  Adds:   desktop environment stack (Xvfb, xfce4, x11vnc, novnc,")
    print("          Firefox) required by Daytona Computer Use, on top of the")
    print("          existing pre-baked Next.js template.\n")
    ...
    print(f"   Snapshot name: {snapshot_name}  (old snapshot '{OLD_SNAPSHOT_NAME}' is left untouched)")
    ...
    print(f"\n  Next steps:")
    print(f"    1. Test: create a sandbox from '{snapshot_name}' and confirm both")
    print(f"       (a) the Next.js dev server boots normally, and")
    print(f"       (b) sandbox.computerUse.start() succeeds (no more 503).")
    print(f"    2. Once verified, update packages/shared/src/constants.ts:")
    print(f'         DAYTONA_SNAPSHOT_NAME = process.env.DAYTONA_SNAPSHOT_NAME || "{snapshot_name}"')
    print(f"    3. Only then delete the old '{OLD_SNAPSHOT_NAME}' snapshot.")
    print()
